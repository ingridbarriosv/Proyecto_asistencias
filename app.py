...
if st.session_state.guardado is False:
    if st.button("Guardar Asistencia"):
        if not tienda or not codigo_opcion:
            ...
            st.warning("Por favor escribe tu nombre real antes de guardar.")
        elif venta == 0:
            ...
            archivo = "data/asistencias.csv"
            if os.path.exists(archivo):
                df_existente = pd.read_csv(archivo)
                duplicado = (
                    (df_existente["Codigo_Num"] == codigo_num) &
                    (df_existente["Fecha"] == str(fecha))
                ).any()
                if duplicado:
